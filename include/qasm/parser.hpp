...
// TODO: error recovery.
  ...
  private:
    ...
    /*! \brief Parse a list of access paths (<anylist>) */
    // <anylist> = <idlist> | <mixedlist>
    // <mixedlist> = <id> [ <nninteger> ] | <mixedlist> , <id>
    //             | <mixedlist> , <id> [ <nninteger> ]
    //             | <idlist> , <id>[ <nninteger> ]
    list_aps* parse_aplist()
    {
      auto builder = list_aps::builder(context_.get(), current_token_.location);
      do {
        auto arg = parse_accesspath();
    ...
    // <argument> = <id>
    //            | <id> [ <nninteger> ]
    ast_node* parse_accesspath()
    {
      auto location = current_token_.location;
      auto identifier = expect_and_consume_token(token_kinds::identifier);
      if (!check_decl(identifier.location, identifier)) {
        return nullptr;
      }
      if (not try_and_consume_token(token_kinds::l_square)) {
        return expr_var::build(context_.get(), identifier.location, identifier);
      }

      auto idx = expect_and_consume_token(token_kinds::nninteger);
      auto expr = expr_integer::create(context_.get(), idx.location, idx);
      expect_and_consume_token(token_kinds::r_square);

      if (!error_) {
        return expr_reg_offset::build(context_.get(), identifier.location, identifier, expr);
      }
    ...
    // <explist> = <exp> 
    //           | <explist> , <exp>
    list_exprs* parse_explist()
    {
      auto builder = list_exprs::builder(context_.get(), current_token_.location);
      do {
        auto expr = parse_exp();
...
#pragma region Helper functions
    stmt_cnot* parse_cnot()
    {
      ...
      auto location = current_token_.location;
      consume_token();
      auto arg1 = parse_accesspath();
      expect_and_consume_token(token_kinds::comma);
      auto arg2 = parse_accesspath();
      expect_and_consume_token(token_kinds::semicolon);
      if (not error_) {
    ...
    ast_node* parse_atom()
    {
      ...
      switch (current_token_.kind) {
      case token_kinds::identifier:
        if (!check_decl(current_token_.location, current_token_)) {
          atom = nullptr;
        } else {
          atom = expr_var::build(context_.get(), current_token_.location, current_token_);
        }
        break;
    ...
    stmt_gate* parse_gate_statement()
    {
      ...
      auto stmt_builder = stmt_gate::builder(context_.get(), identifier.location, identifier);

      // Parse classical args
      if (try_and_consume_token(token_kinds::l_paren)) {
        if (not try_and_consume_token(token_kinds::r_paren)) {
          stmt_builder.add_cargs(parse_explist());
          expect_and_consume_token(token_kinds::r_paren);
        }
      }

      // Parse quantum args
      stmt_builder.add_qargs(parse_aplist());
      expect_and_consume_token(token_kinds::semicolon);
	...
	stmt_barrier* parse_barrier()
	{
      ...
      consume_token();
      stmt_builder.add_child(parse_aplist());
      expect_and_consume_token(token_kinds::semicolon);
      return stmt_builder.finish();
	}
	...
	stmt_if* parse_if()
	{
      ...
      auto var = expr_var::build(context_.get(), identifier.location, identifier);
      ...
      expect_and_consume_token(token_kinds::r_paren);
      stmt_builder.add_child(var);
      stmt_builder.add_child(atom);
      stmt_builder.add_child(parse_qop());
      return stmt_builder.finish();
	}

	bool check_decl(uint32_t location, std::string_view identifier)
	{
