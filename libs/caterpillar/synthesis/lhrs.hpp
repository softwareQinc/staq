...
private:
  ...
  void prepare_constant( bool value )
  {
    ...
    qnet.add_qubit();
    if ( v )
      qnet.add_gate( tweedledum::gate::pauli_x, tweedledum::io_id(node_to_qubit[n], true) );
  }
  ...
  void prepare_outputs()
  {
    ...
    ntk.foreach_po( [&]( auto s ) {
      ...
      if ( const auto it = node_to_signals.find( node ); it != node_to_signals.end() ) //node previously referred
      {
        ...
        qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id(node_to_qubit[ntk.node_to_index( node )], true), tweedledum::io_id(new_i, true) );
        if ( ntk.is_complemented( s ) != ntk.is_complemented( node_to_signals[node] ) )
        {
          qnet.add_gate( tweedledum::gate::pauli_x, tweedledum::io_id(new_i, true));
        }
      ...
      else //node never referred
      {
        ...
        {
          qnet.add_gate( tweedledum::gate::pauli_x, tweedledum::io_id(node_to_qubit[ntk.node_to_index( node )], true));
        }
  ...
  SetQubits get_fanin_as_qubits( mt::node<LogicNetwork> const& n )
  {
    ...
    ntk.foreach_fanin( n, [&]( auto const& f ) {
      ...
      controls.push_back( tweedledum::io_id( node_to_qubit[ntk.node_to_index( ntk.get_node( f ) )], true ) );
    } );
    return controls;
  }
  ...
  void compute_big_xor( uint32_t t, std::vector<uint32_t> leaves)
  {
    for ( auto control : leaves )
    {
      ...
      {
        qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( node_to_qubit[control], true ), tweedledum::io_id( t, true ) );
      }
    }
  }
  ...
  void compute_node( mt::node<LogicNetwork> const& node, uint32_t t )
  {
    ...
    if constexpr ( mt::has_node_function_v<LogicNetwork> )
    {
      ...
      else
      {
        ...
        const auto controls = get_fanin_as_qubits( node );
        compute_lut( ntk.node_function( node ), controls, tweedledum::io_id( t, true ) );
      }
    }
  }
  ...
  void compute_node_as_cell( mt::node<LogicNetwork> const& node, uint32_t t, kitty::dynamic_truth_table const& func, std::vector<uint32_t> const& leave_indexes )
  {
    ...
    {
      controls.push_back( tweedledum::io_id( node_to_qubit[ntk.node_to_index( l )], true ) );
    }
  ...
  {
    qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c1, true ), tweedledum::io_id( t, true ) );
    qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c2, true ), tweedledum::io_id( t, true ) );
    if ( inv )
      qnet.add_gate( tweedledum::gate::pauli_x, tweedledum::io_id( t, true ) );
  }
  ...
  void compute_xor3( uint32_t c1, uint32_t c2, uint32_t c3, bool inv, uint32_t t )
  {
    qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c1, true ), tweedledum::io_id( t, true ) );
    qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c2, true ), tweedledum::io_id( t, true ) );
    qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c3, true ), tweedledum::io_id( t, true ) );
    if ( inv )
      qnet.add_gate( tweedledum::gate::pauli_x, tweedledum::io_id( t, true ) );
  }
  ...
  void compute_xor_block( SetQubits const& controls, Qubit t )
  {
    ...
    {
      if ( c != t )
        qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c, true ), t );
    }
  }
  ...
  void compute_xor_inplace( uint32_t c1, uint32_t c2, bool inv, uint32_t t )
  {
    if ( c1 == t && c2 != t)
    {
      qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c2, true ), c1 );
    }
    else if ( c2 == t && c1 !=t)
    {
      qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c1, true ), c2 );
  ...
  void compute_xor3_inplace( uint32_t c1, uint32_t c2, uint32_t c3, bool inv, uint32_t t )
  {
    if ( c1 == t )
    {
      qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c2, true ), tweedledum::io_id( c1, true ) );
      qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c3, true ), tweedledum::io_id( c1, true ) );
    }
    else if ( c2 == t )
    {
      qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c1, true ), c2 );
      qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c3, true ), c2 );
    }
    else if ( c3 == t )
    {
      qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c1, true ), c3 );
      qnet.add_gate( tweedledum::gate::cx, tweedledum::io_id( c2, true ), c3 );
    }
    else
    {
      //std::cerr << "[e] target does not match any control in in-place\n";
