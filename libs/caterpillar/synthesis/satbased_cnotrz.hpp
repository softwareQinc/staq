...
{
public:
  satbased_cnotrz_impl( bit_matrix_rm<> const& transform, parity_terms const& parities, satbased_cnotrz_params const& ps, satbased_cnotrz_stats& st )
      : num_qubits( transform.num_rows() ),
        transform( transform ),
        parities( parities ),
        ps( ps ),
        st( st ),
        offset( num_qubits * num_qubits + 2 * num_qubits + parities.num_terms() * ( num_qubits + 1 ) )
  {
  }
  ...
  Network run()
  {
    ...
    while ( true )
    {
      ...
      const auto result = mockturtle::call_with_stopwatch( st.time_solving, [&]() { return solver.solve( &assumps[0], &assumps[0] + assumps.size(), ps.conflict_limit ); } );
...
private:
  void assert_initial()
  {
    for ( auto row = 0u; row < num_qubits; ++row )
    {
      ...
      {
        int lit = pabc::Abc_Var2Lit( matrix_var( 0, row, column ), row != column );
  ...
  std::vector<int> assume_final( uint32_t time )
  {
    ...
    for ( auto row = 0u; row < num_qubits; ++row )
    {
      for ( auto column = 0u; column < num_qubits; ++column )
      {
        assumptions.push_back( pabc::Abc_Var2Lit( matrix_var( time, row, column ), 1 - transform.at( row, column ) ) );
      }
    }

    for ( auto id = 0u; id < parities.num_terms(); ++id )
    {
      assumptions.push_back( pabc::Abc_Var2Lit( parity_term_var( time, id ), 0 ) );
    }
    return assumptions;
  }
  ...
  void assert_transition( uint32_t time )
  {
    ...
    for ( auto row = 0u; row < num_qubits; ++row )
    {
      for ( auto column = 0u; column < num_qubits; ++column )
      {
        ...
        clause[2] = pabc::Abc_Var2Lit( matrix_var( time + 1, row, column ), 0 );
        solver.add_clause( clause, clause + 3 );

        clause[1] = pabc::Abc_Var2Lit( matrix_var( time, row, column ), 0 );
        clause[2] = pabc::Abc_Var2Lit( matrix_var( time + 1, row, column ), 1 );
        solver.add_clause( clause, clause + 3 );
        ...
        for ( auto rr = 0u; rr < num_qubits; ++rr )
        {
          ...
          clause[2] = pabc::Abc_Var2Lit( matrix_var( time, row, column ), 1 );
          clause[3] = pabc::Abc_Var2Lit( matrix_var( time, rr, column ), 1 );
          clause[4] = pabc::Abc_Var2Lit( matrix_var( time + 1, row, column ), 1 );
          solver.add_clause( clause, clause + 5 );

          clause[2] = pabc::Abc_Var2Lit( matrix_var( time, row, column ), 1 );
          clause[3] = pabc::Abc_Var2Lit( matrix_var( time, rr, column ), 0 );
          clause[4] = pabc::Abc_Var2Lit( matrix_var( time + 1, row, column ), 0 );
          solver.add_clause( clause, clause + 5 );

          clause[2] = pabc::Abc_Var2Lit( matrix_var( time, row, column ), 0 );
          clause[3] = pabc::Abc_Var2Lit( matrix_var( time, rr, column ), 1 );
          clause[4] = pabc::Abc_Var2Lit( matrix_var( time + 1, row, column ), 0 );
          solver.add_clause( clause, clause + 5 );
  ...
  void assert_symmetry_break_matrix( uint32_t time )
  {
    ...
    // NOTE
    /* With both symmetry breaks enabled, the runtime gets worse. With only one
       break enabled, runtime improves, slightly better with the second one.
       For larger instances it may be beneficial to have both breaks in place.
    */
    ...
    /*for ( auto row = 0u; row < num_qubits; ++row )
    {
      for ( auto column = 0u; column < num_qubits; ++column )
      {
        clause[column] = pabc::Abc_Var2Lit( matrix_var( time, row, column ), 0 );
      }
    ...
    for ( auto column = 0u; column < num_qubits; ++column )
    {
      for ( auto row = 0u; row < num_qubits; ++row )
      {
        clause[row] = pabc::Abc_Var2Lit( matrix_var( time, row, column ), 0 );
      }
      solver.add_clause( &clause[0], &clause[0] + num_qubits );
    }
  }
  ...
  void assert_symmetry_break( uint32_t time )
  {
    ...
    for ( auto row = 0u; row < num_qubits; ++row )
    {
      for ( auto rr2 = 1u; rr2 < num_qubits; ++rr2 )
      {
        ...
        for ( auto rr1 = 0u; rr1 < rr2; ++rr1 )
        {
          ...
          clause[0] = pabc::Abc_Var2Lit( control_var( time - 1, row ), 1 );
    ...
    /* first control can only be larger if targets are blocking (doesn't work) */
    //for ( auto rr2 = 1u; rr2 < num_qubits; ++rr2 )
    //{
    //  for ( auto rr1 = 0u; rr1 < rr2; ++rr1 )
    //  {
    ...
    for ( auto const& [term, _] : parities )
    {
      ...
      for ( auto row = 0u; row < num_qubits; ++row )
      {
        for ( auto column = 0u; column < num_qubits; ++column )
        {
          ...
          and_inputs[column] = pabc::Abc_Var2Lit( matrix_var( time, row, column ), 1 - pol );
        }
        assert_and( and_inputs, parity_term_row_var( time, ctr, row ) );
        or_inputs[row] = pabc::Abc_Var2Lit( parity_term_row_var( time, ctr, row ), 0 );
      }

      /* OR of all parity vars */
      if ( time > 0 )
      {
        or_inputs[num_qubits] = pabc::Abc_Var2Lit( parity_term_var( time - 1, ctr ), 0 );

        /* propagate parity term solution */
        clause[0] = pabc::Abc_Var2Lit( parity_term_var( time - 1, ctr ), 1 );
  ...
  Network extract_solution( uint32_t time )
  {
    ...
    for ( auto i = 0u; i < num_qubits; ++i )
    {
      ...
      if ( auto rotation = parities.extract_term( qubits_states[i] ); rotation != 0.0 )
      {
    ...
    for ( auto i = 0u; i < time; ++i )
    {
      ...
      if ( auto rotation = parities.extract_term( qubits_states[t] ); rotation != 0.0 )
      {
...
private:
  ...
  {
    return time * offset + num_qubits * num_qubits + 2 * num_qubits + id * ( num_qubits + 1 );
  }

  inline int parity_term_row_var( uint32_t time, uint32_t id, uint32_t row ) const
  {
    return time * offset + num_qubits * num_qubits + 2 * num_qubits + id * ( num_qubits + 1 ) + 1 + row;
  }
...
template<class Network>
Network satbased_cnotrz( bit_matrix_rm<> const& transform, parity_terms const& parities, satbased_cnotrz_params const& ps = {} )
{
  satbased_cnotrz_stats st;
