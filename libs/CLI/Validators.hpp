...
std::string generate_set(const T& set) {
    ...
    out.append(detail::join(detail::smart_deref(set),
                            [](const iteration_type_t& v) {
                                return detail::pair_adaptor<element_t>::first(
                                    v);
                            },
                            ","));
    out.push_back('}');
...
std::string generate_map(const T& map) {
    ...
    std::string out(1, '{');
    out.append(
        detail::join(detail::smart_deref(map),
                     [](const iteration_type_t& v) {
                         return detail::as_string(
                                    detail::pair_adaptor<element_t>::first(v)) +
                                "->" +
                                detail::as_string(
    ...
    explicit CheckedTransformer(T mapping, F filter_function) {
        ...
        func_ = [mapping, tfunc, filter_fn](std::string& input) {
            ...
            for (const auto& v : detail::smart_deref(mapping)) {
                auto output_string = detail::as_string(
                    detail::pair_adaptor<element_t>::second(v));
