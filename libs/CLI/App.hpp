...
class App {
  ...
  protected:
    ...
    /// If set to true the subcommand will be reenabled at the start of each
    /// parse
  ...
  public:
    ...
    Option* add_set(std::string option_name,
        ...
        Option* opt = add_option(option_name, member,
                                 std::move(option_description), defaulted);
        opt->check(IsMember{options});
        return opt;
    }
    ...
    /// DEPRECATED
    CLI11_DEPRECATED(
        "Use ->transform(CLI::IsMember(..., CLI::ignore_case)) instead")
    ...
    /// Add set of options, string only, ignore case (default, set can be
    /// changed afterwards - do not destroy the set) DEPRECATED
    CLI11_DEPRECATED(
        "Use ->transform(CLI::IsMember(...)) with a (shared) pointer instead")
    Option* add_mutable_set_ignore_case(
        std::string option_name,
        std::string& member, ///< The selected member of the set
        const std::set<std::string>& options, ///< The set of possibilities
    ...
    /// set) DEPRECATED
    CLI11_DEPRECATED(
        "Use ->transform(CLI::IsMember(..., CLI::ignore_underscore)) instead")
    Option* add_set_ignore_underscore(
        std::string option_name,
        std::string& member,           ///< The selected member of the set
        std::set<std::string> options, ///< The set of possibilities
        std::string option_description = "") {
    ...
    /// Add set of options, string only, ignore underscore (no default, set can
    /// be changed afterwards - do not destroy the set) DEPRECATED
    ...
    Option* add_set_ignore_case_underscore(
        ...
        Option* opt =
            add_option(option_name, member, std::move(option_description));
        opt->transform(
            IsMember{options, CLI::ignore_underscore, CLI::ignore_case});
        return opt;
    }
    ...
    Option* add_complex(std::string option_name, T& variable,
        ...
        CLI::Option* opt = add_option(option_name, std::move(fun),
                                      std::move(option_description), defaulted,
                                      default_function);
    ...
    App* add_subcommand(CLI::App_p subcom) {
        ...
        if (!subcom->name_.empty()) {
            for (const auto& subc : subcommands_)
                if (subc->check_name(subcom->name_) ||
                    subcom->check_name(subc->name_))
                    throw OptionAlreadyAdded(subc->name_);
        }
        subcom->parent_ = this;
    ...
    size_t count_all() const {
        ...
        if (!get_name().empty()) { // for named subcommands add the number of
                                   // times the subcommand was called
            cnt += parsed_;
        }
        return cnt;
    }
  ...
  protected:
        ...
        if (require_option_min_ > 0) {
            if (require_option_max_ > 0) {
                if (require_option_max_ < require_option_min_) {
                    throw(InvalidError("Required min options greater than "
                                       "required max options",
                                       ExitCodes::InvalidError));
                }
            }
            if (require_option_min_ > (options_.size() + nameless_subs)) {
                throw(InvalidError("Required min options greater than number "
    ...
    /// fails, add to missing if missing from master return false if the parse
    /// has failed and needs to return to parent
    ...
    bool _parse_positional(std::vector<std::string>& args) {
        ...
        auto parent_app =
            (parent_ != nullptr) ? _get_fallthrough_parent() : this;
    ...
    bool _parse_arg(std::vector<std::string>& args,
        ...
        switch (current_type) {
            ...
            case detail::Classifier::WINDOWS:
                if (!detail::split_windows_style(current, arg_name, value))
                    throw HorribleError("windows option parsed but missing! "
            ...
            case detail::Classifier::NONE:
            default:
                throw HorribleError("parsing got called with invalid option! "
                                    "You should not see this");
        }
        ...
        if (num < 0) {
            ...
            if (num > 0) {
                throw ArgumentMismatch::TypedAtLeast(op->get_name(), num,
                                                     op->get_type_name());
            }
        }
