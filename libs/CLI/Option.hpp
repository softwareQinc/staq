...
class Option : public OptionBase<Option> {
  ...
  public:
    ...
    /// The default value (for help printing) DEPRECATED Use get_default_str()
    /// instead
    ...
    void run_callback() {
        ...
        if (multi_option_policy_ == MultiOptionPolicy::TakeLast) {
            ...
            results_t partial_result{results_.end() - trim_size,
                                     results_.end()};
            ...
            if (get_items_expected() > 0) {
                if (results_.size() !=
                    ...
                    throw ArgumentMismatch(get_name(), get_items_expected(),
                                           results_.size());
    ...
    std::string get_flag_value(std::string name,
        ...
        if (disable_flag_override_) {
            if (!((input_value.empty()) || (input_value == emptyString))) {
                ...
                if (default_ind >= 0) {
                    if (default_flag_values_[default_ind].second !=
                        input_value) {
        ...
        if (results_.empty()) {
            ...
            switch (multi_option_policy_) {
                ...
                case MultiOptionPolicy::Join:
                    retval =
                        detail::lexical_cast(detail::join(results_), output);
                    break;
            }
        }
        if (!retval) {
    ...
    /// Set the default value string representation and evaluate into the bound
    /// value
