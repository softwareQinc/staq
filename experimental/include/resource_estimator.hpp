/**
 * \file resource_estimator.hpp
 * \brief Resource estimation
 */

#ifndef RESOURCE_ESTIMATOR_HPP_
#define RESOURCE_ESTIMATOR_HPP_
...
namespace ast = tools_v1::ast;

using resource_count = std::unordered_map<std::string, double>;
...
public:
  ...
  resource_count run(ast::ASTNode &node) {
    ...
    // Set depth and add any improved T counts
    counts["depth"] = depth;
    finalize_t_counts(counts);
    return counts;
  }
  ...
  void visit(ast::PauliString&) {
    throw "not supported";
  }
  void visit(ast::PhaseGate&) {
    throw "not supported";
  }
  void visit(ast::ExpPauli&) {
    throw "not supported";
  }
  void visit(ast::ControlGate&) {
    throw "not supported";
  }
  void visit(ast::MultiControlGate&) {
    throw "not supported";
  }
...
private:
  ...
  void reset() {
    ...
    rotation_gate_cost_ =
        config_.rotation_precision
            ? predict_t_count(std::max(0.0, *config_.rotation_precision))
            : 0.0;
  }
  ...
  static double predict_t_count(double precision) {
    const double SLOPE = 10.03;
    const double INTERCEPT = 0.21;
    return std::ceil((SLOPE * precision) + INTERCEPT);
  }

  void track_rotation_gate(const std::string &name) {
    if (!config_.rotation_precision || rotation_gate_cost_ <= 0.0) {
      return;
    }
    auto base = base_gate_name(name);
    if (base == "rx" || base == "ry" || base == "rz") {
      running_estimate_.first["t_rotations"] += rotation_gate_cost_;
    }
  }

  std::string_view base_gate_name(const std::string &name) const {
  ...
  void finalize_t_counts(resource_count &counts) {
    if (!config_.rotation_precision) {
      return;
    }
    counts["t_improved"] = counts["t"] + counts["t_rotations"];
  }

  double rotation_gate_cost_ = 0.0;
};
